...
namespace cyng {
    namespace crypto {
        ...
        bool generate_x509_cert_request(
        ...
        EVP_PKEY_ptr generate_private_key(int bits) {
            ...
            // Elliptic Curve algorithm P-256 is one of the curves recommended by NIST
            auto ec_key = create_ec_key_by_curve_name(NID_secp256k1);
            // EC_KEY *key_pair_obj = EC_KEY_new_by_curve_name(NID_secp256k1);
            int ret_error = EC_KEY_generate_key(ec_key.get());

            // Get private key
            BIGNUM *priv_key = (BIGNUM *)EC_KEY_get0_private_key(ec_key.get());
            char *priv_key_char = BN_bn2hex(priv_key);

            return std::unique_ptr<EVP_PKEY, decltype(&::EVP_PKEY_free)>(nullptr, [](EVP_PKEY *) {});
#endif
        }

        bool generate_priv_pub_key_pair(const char *pub_pem, const char *priv_pem, int bits) {
            // 1. generate rsa key with exponent RSA_F4
            auto bnp = create_bignum_rsa_f4();
            if (!bnp)
                return false;
            ...
            // Elliptic Curve algorithm P-256 is one of the curves recommended by NIST
            EC_KEY *key_pair_obj = EC_KEY_new_by_curve_name(NID_secp256k1);
            int ret_error = EC_KEY_generate_key(key_pair_obj);

            // Get private key
            BIGNUM *priv_key = (BIGNUM *)EC_KEY_get0_private_key(key_pair_obj);
            char *priv_key_char = BN_bn2hex(priv_key);

            EC_POINT *pub_key = (EC_POINT *)EC_KEY_get0_public_key(key_pair_obj);
            EC_GROUP *secp256k1_group = EC_GROUP_new_by_curve_name(NID_secp256k1);
            char *pub_key_char = EC_POINT_point2hex(secp256k1_group, pub_key, POINT_CONVERSION_UNCOMPRESSED, nullptr);
            ...
            //  use i2d_ECPrivateKey_bio() or i2d_ECPrivateKey_fp() to write into a file
